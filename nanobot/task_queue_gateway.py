...
在 nanobot gateway 启动时调用此函数来初始化任务队列：
    from nanobot.task_queue_gateway import integrate_task_queue
...
def create_task_command(task_queue):
    ...
    def submit_and_notify(
        ...
        task_id = task_queue.submit(name=name, metadata=metadata, user_id=user_id, channel=channel)
        ...
        if wait:
            ...
            return {"task_id": task_id, "status": "timeout", "message": f"等待超时 ({timeout}秒)"}
...
def submit_task(
    ...
    用法:
        from nanobot.task_queue_gateway import submit_task
